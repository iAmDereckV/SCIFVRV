...
class Dashboard
{
    ...
    public function resumen()
    {
        $ventasMes = $this->conexion
            ->query("SELECT IFNULL(SUM(total),0) total
        FROM ventas
        WHERE estado='COMPLETADA'
        AND MONTH(fecha)=MONTH(CURDATE())
        AND YEAR(fecha)=YEAR(CURDATE())")->fetch();
        ...
        $gastosMes = $this->conexion
            ->query("SELECT IFNULL(SUM(monto),0) total
        FROM gastos
        ...
        AND estado='ACTIVO'")->fetch();
        $ventas = $this->conexion
            ->query("SELECT
                IFNULL(SUM(total),0) total
                FROM ventas
                WHERE DATE(fecha)=CURDATE()")->fetch();
        $productos = $this->conexion
            ->query("SELECT COUNT(*) total
                FROM productos")->fetch();
        $clientes = $this->conexion
            ->query("SELECT COUNT(*) total
                FROM clientes")->fetch();
        $facturas = $this->conexion
            ->query("SELECT COUNT(*) total
                FROM ventas")->fetch();
        return [
            ...
            'utilidad' => $ventasMes['total'] - $gastosMes['total'],
    ...
    FROM
    (SELECT DATE_SUB(CURDATE(),INTERVAL n MONTH) mes
    FROM (SELECT 0 n UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) x ) meses 
    LEFT JOIN (
    SELECT YEAR(fecha) anio, MONTH(fecha) mes, SUM(total) total_ventas
    FROM ventas
    WHERE estado='COMPLETADA'
    GROUP BY YEAR(fecha),MONTH(fecha)) v
    ON YEAR(meses.mes)=v.anio
    AND MONTH(meses.mes)=v.mes
    LEFT JOIN
    (SELECT YEAR(fecha) anio,MONTH(fecha) mes,SUM(total) total_compras
        FROM compras
        WHERE estado='COMPLETADA'
        GROUP BY YEAR(fecha),MONTH(fecha)) c
    ...
    {
        $sql = "SELECT p.nombre,SUM(d.cantidad) cantidad
    ...
    {
        $sql = "SELECT u.nombre,SUM(v.total) total
    FROM ventas v
    ...
    public function resumenFinanciero()
    {
        ...
        $costos = $this->conexion
            ->query("SELECT IFNULL(SUM(costo_unitario * cantidad),0) total
            FROM detalle_ventas dv
            INNER JOIN ventas v ON v.id = dv.venta_id
            WHERE v.estado='COMPLETADA'
            AND MONTH(v.fecha)=MONTH(CURDATE())
            AND YEAR(v.fecha)=YEAR(CURDATE())")->fetch();
        // Gastos del mes
    ...
    {
        $sql = "SELECT v.id,c.nombres cliente,v.total
        FROM ventas v
    ...
    {
        $sql = "SELECT c.id,p.nombre proveedor,c.total
        FROM compras c
    ...
    {
        $sql = "SELECT nombre,usuario,ultimo_acceso
        FROM usuarios
        WHERE estado='ACTIVO'
