...
class Kardex
{
    ...
    public function consultar($producto_id)
    {
        $sql = "SELECT
            ...
            fecha AS fecha_orden,'COMPRA' tipo,cantidad,
            CONCAT('Compra #',compra_id) detalle
        FROM detalle_compras dc
        INNER JOIN compras c ON c.id = dc.compra_id
        WHERE producto_id = :producto_id
        UNION ALL
        SELECT DATE_FORMAT(fecha, '%d/%m/%Y %H:%i:%s') AS fecha,
            fecha AS fecha_orden,'VENTA' tipo,cantidad,
            CONCAT('Venta #',venta_id) detalle
    ...
    {
        $sql = "SELECT codigo,nombre,stock,stock_minimo,imagen,estado
            FROM productos
            WHERE id = :id";
        $stmt = $this->conexion->prepare($sql);
        $stmt->execute([':id' => $id]);
