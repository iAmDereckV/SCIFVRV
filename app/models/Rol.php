...
class Rol
{
    ...
    {
        $sql = "SELECT p.id,p.codigo,p.descripcion,
            IF(rp.permiso_id IS NULL,0,1) asignado
        FROM permisos p
        LEFT JOIN rol_permisos rp ON rp.permiso_id = p.id AND rp.rol_id = :rol_id
        ORDER BY p.codigo";
        $stmt = $this->conexion->prepare($sql);
        $stmt->execute([':rol_id' => $rol_id]);
        return $stmt->fetchAll();
    }
    public function guardarPermisos($rol_id, $permisos)
    {
        $this->conexion->beginTransaction();
        try {
            $sql = "DELETE
